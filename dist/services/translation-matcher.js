...
export class TranslationMatcherService {
     ...
     */
    findWordCombinationMatch(koreanText, wordSection, enWordSection) {
        // 공백으로 단어 분리
     ...
     * 객체에서 값으로 키 경로를 찾습니다
     */
    findKeyByValue(obj, targetValue, currentPath = '') {
    ...
    isMatch(value, target) {
        ...
        if (value === target)
            return true;
        // 공백 제거 후 매치
        if (value.replace(/\s/g, '') === target.replace(/\s/g, ''))
            return true;
     ...
     * 매칭 신뢰도 계산
     */
    calculateConfidence(target, found) {
        if (target === found)
            return 1.0;
        if (target.replace(/\s/g, '') === found.replace(/\s/g, ''))
            return 0.95;
     ...
     */
    async getUnmatchedTexts(koreanTexts) {
...
//# sourceMappingURL=translation-matcher.js.map