...
import { DEFAULT_I18N_CONFIG, VUE_I18N_CONFIGS } from '../../types/i18n-config.js';
/**
 * 한글 대체 처리 메인 도구
 */
export class ProcessKoreanReplacementTool extends BaseTool {
    constructor(context) {
        super(context);
        this.name = 'process-korean-replacement';
        this.description = 'Vue/JS/TS 파일에서 한글 텍스트를 자동으로 감지하고 분석하여 i18n 키 대체 준비를 수행합니다';
        this.inputSchema = {
            type: 'object',
            properties: {
                fileName: {
    ...
    async execute(input) {
        ...
        try {
            ...
            // 한글 패턴 추출
            const extractions = await this.extractKoreanPatterns(input.fileName, input.fileContent, fileType);
            // 총 추출된 한글 텍스트 수 계산
     ...
     * 파일 타입 자동 감지
     */
    detectFileType(fileName, explicitType) {
        if (explicitType) {
            return explicitType;
        }
     ...
     * 파일별 한글 패턴 추출
     */
    async extractKoreanPatterns(fileName, content, fileType) {
        const extractions = {};
        switch (fileType) {
            case 'vue':
     ...
     * 전체 추출 개수 계산
     */
    countTotalExtractions(extractions) {
        let total = 0;
        if (extractions.vue)
            total += extractions.vue.length;
        if (extractions.js)
            total += extractions.js.length;
        return total;
    }
    /**
     * 모든 한글 텍스트를 배열로 추출
     */
    extractAllKoreanTexts(extractions) {
        const texts = [];
        if (extractions.vue) {
     ...
     * 추천사항 생성
     */
    generateRecommendations(extractions, fileType, translationMatches = [], unmatchedTexts = []) {
     ...
     * 매칭된 키 찾기 헬퍼 메서드
     */
    findMatchedKey(koreanText, translationMatches) {
     ...
     * 변환 예시를 생성하는 헬퍼 메서드
     */
    generateConversionExample(text, keyPath, functionName) {
     ...
     * 다음 단계 안내 생성
     */
    generateNextSteps(totalKoreanTexts, matchedCount = 0) {
        const nextSteps = [];
        if (totalKoreanTexts === 0) {
...
//# sourceMappingURL=process-korean-replacement.js.map