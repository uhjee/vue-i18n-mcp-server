...
export class VueI18nMCPServer {
    constructor() {
        ...
        this.server = new Server({
            ...
            version: '1.0.0',
            description: 'Vue.js i18n automation server for GitHub Copilot Agent Mode',
        }, {
            capabilities: {
                tools: {
                // 도구 기능 활성화
     ...
     */
    findProjectsInDirectory(basePath, maxDepth = 2) {
        const projects = [];
        try {
    ...
    initializeTools() {
        ...
        // RFP 1단계: 메인 처리 도구 등록
        this.tools.set('process-korean-replacement', new ProcessKoreanReplacementTool(toolContext));
        // RFP 3단계: 기존 번역 매칭 도구 등록
        this.tools.set('find-existing-translations', new FindExistingTranslationsTool(toolContext));
    }
    /**
     * MCP 요청 핸들러 설정
     */
    ...
    formatProcessResult(result) {
        ...
        if (result.summary.totalKoreanTexts > 0) {
            ...
            if (result.translationMatches && result.translationMatches.length > 0) {
                ...
                result.translationMatches.slice(0, 5).forEach((match, index) => {
            ...
            if (result.unmatchedTexts && result.unmatchedTexts.length > 0) {
                ...
                result.unmatchedTexts.slice(0, 5).forEach((text, index) => {
    ...
    formatMatchingResult(result) {
        ...
        if (result.unmatched && result.unmatched.length > 0) {
            ...
            result.unmatched.slice(0, 10).forEach((text, index) => {
                formatted += `${index + 1}. "${text}"\n`;
            });
